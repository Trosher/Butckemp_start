...
void max_x(int *data, int x);
void min_y(int **data, int y, int x, int target);
...
void min_y(int **data, int y, int x, int target) {
    int min = 2147483647;
    int *p;
    if (target == 1) { p = (int*)data; }
    for (int i = 0; i < x; i++) {
        for (int j = 0; j < y; j++) {
            if (target == 1) {
                if (min > p[j * y + i]) { min = p[j * y + i]; }
            } else {
                if (min > data[j][i]) { min = data[j][i]; }
            }
        }
        if (i < x - 1) { printf("%d ", min);
            min = 2147483647;
        } else { printf("%d", min); }
    }
}
